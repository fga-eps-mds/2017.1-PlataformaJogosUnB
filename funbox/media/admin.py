...
def save_object(instance, list_file):
    ...
    if type(instance) == Image:
        for archive in list_file:
            instance = type(instance).objects.create(game=game,role=role,image=archive)
            instance.save()
    elif type(instance) == Video:
        for archive in list_file:
            instance = type(instance).objects.create(game=game,role=role,video=archive)
            instance.save()
    elif type(instance) == Soundtrack:
        for archive in list_file:
            instance = type(instance).objects.create(game=game,role=role,soundtrack=archive)
            instance.save()
