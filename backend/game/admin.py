...
class GameAdmin(admin.ModelAdmin):
    ...
    def save_formset(self, request, form, formset, change):
        ...
        for forms in formset:
            instances.append(forms.save(commit=False))
        print(change)
        print(instances)
        print(form.cleaned_data)
        list_files = request.FILES.getlist('media_image-0-image')
        print(list_files)
        print(formset[1].cleaned_data)
        formset[0].save_instances(list_files)
        # if instances:
